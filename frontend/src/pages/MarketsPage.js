...
const MarketsPage = ({ user }) => {
  ...
  // Load which symbols are already in the user's watchlist (so the star reflects state)
  useEffect(() => {
    const loadWatchlist = async () => {
      try {
        const res = await watchlistAPI.get(user?.id);
        setWatchlisted(new Set((res.data?.data || []).map(w => w.stock_symbol)));
      } catch (err) {
        // non-critical
      }
    };
  ...
  const handleToggleWatchlist = async (e, symbol) => {
    e.stopPropagation();
    const isIn = watchlisted.has(symbol);
    try {
      if (isIn) {
        await watchlistAPI.remove(symbol);
        setWatchlisted(prev => { const next = new Set(prev); next.delete(symbol); return next; });
        setNotice(`${symbol} removed from watchlist`);
      } else {
        await watchlistAPI.add(symbol);
        setWatchlisted(prev => new Set(prev).add(symbol));
        setNotice(`${symbol} added to watchlist`);
      }
    } catch (err) {
      setNotice(err.response?.data?.message || `Could not update ${symbol}`);
    } finally {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      {notice && (
        <div className="fixed bottom-6 right-6 z-50 bg-gray-900 text-white text-sm font-bold px-4 py-3 rounded-xl shadow-lg animate-in slide-in-from-bottom-2">
          {notice}
        </div>
      )}
                            ...
                            <div className="pt-4 mt-auto border-t border-gray-50 flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2 z-10">
                                    <button
                                        ...
                                        title={watchlisted.has(stock.symbol) ? 'Remove from watchlist' : 'Add to watchlist'}
                                        className={`p-1.5 rounded-lg transition-colors hover:bg-yellow-50 ${watchlisted.has(stock.symbol) ? 'text-yellow-400' : 'text-gray-300 hover:text-yellow-400'}`}
                                    >
                                        <Star className={`w-4 h-4 ${watchlisted.has(stock.symbol) ? 'fill-yellow-400' : ''}`} />
                                    </button>
