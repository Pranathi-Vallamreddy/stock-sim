...
const PortfolioPage = ({ user, onBalanceChange }) => {
  ...
  // Net worth = cash + what you'd get back by closing every open position
  // (reserved capital entry*qty + its unrealized P&L).
  const openPositionsValue = open_trades.reduce(
    (sum, t) => sum + parseFloat(t.entry_price) * t.quantity + parseFloat(t.unrealized_pnl || 0),
    0
  );
  const portfolioValue = parseFloat(portfolio.user.balance) + openPositionsValue;
