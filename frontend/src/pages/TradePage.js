...
const TradePage = ({ user, onBalanceChange }) => {
  ...
  // Current price comes from the live stocks list (updated by volatilityService)
  useEffect(() => {
    const match = availableStocks.find(
      s => s.symbol === form.stock_symbol.toUpperCase()
    );
    setPrice(match ? parseFloat(match.price) : null);
  }, [form.stock_symbol, availableStocks]);
  ...
  // Opening any position reserves entry * qty of capital, so both BUY and SELL need funds
  const isAffordable = estimatedCost <= parseFloat(user.balance);
