...
const placeTrade = async (req, res) => {
  try {
    ...
    // Get current price from the live stocks table (updated by volatilityService)
    const priceResult = await query(
      `SELECT price FROM stocks WHERE symbol = $1 AND is_active = true`,
    ...
    const entry_price = parseFloat(priceResult.rows[0].price);
    ...
    // Opening any position (long BUY or short SELL) reserves entry_price * qty
    // of capital as margin. This keeps cash conserved: closing returns the
    // reserved capital plus the realized P&L.
    ...
    // Reserve capital for the position (both BUY and SELL)
    await query(
...
const closeTrade = async (req, res) => {
  try {
    ...
    // Get current price (exit price) from the live stocks table
    const priceResult = await query(
      `SELECT price FROM stocks WHERE symbol = $1`,
      [trade.stock_symbol]
    );

    const exit_price = parseFloat(priceResult.rows[0]?.price || trade.entry_price);
    ...
    // Return the reserved capital (entry * qty) plus the realized P&L.
    // For BUY this equals exit * qty; for SELL it correctly settles the short.
    const credit = (entry_price * quantity) + pnl;
...
const getPortfolio = async (req, res) => {
  try {
    ...
    // Get open trades (current price from the live stocks table)
    const openTrades = await query(
      `SELECT t.*, s.price as current_price,
        CASE
          WHEN t.trade_type = 'BUY' THEN (s.price - t.entry_price) * t.quantity
          ELSE (t.entry_price - s.price) * t.quantity
        END as unrealized_pnl
       FROM trades t
       LEFT JOIN stocks s ON s.symbol = t.stock_symbol
